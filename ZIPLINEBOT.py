...
class Portfolio:
    ...
    def update_portfolio_value(self, asset_values):
        self.portfolio_value = self.cash + sum(asset_values)
...
class TradingBot:
    def __init__(self, assets):
        ...
        self.buy_prices = {asset: None for asset in assets}
        self.highest_prices = {asset: 0 for asset in assets}
        self.trade_history = {asset: [] for asset in assets}
        self.portfolio_value_history = []
        self.profit_threshold = 0.015 # This will allow the bot to take profits more frequently, potentially increasing the number of successful trades.
        self.take_profit = 0.03  # 3% take profit
        self.stop_loss = 0.025 # This tighter stop loss may help limit losses on losing trades.
        self.max_position_size = 0.08  # 8% allows for slightly larger positions, which could increase potential profits (but also risks).
        self.trailing_stop_loss = 0.02  # THIS HAS BEEN REMOVED 15% This gives winning trades a bit more room to run before selling.
        self.max_drawdown = 0.35  # Set an initial value
        ...
        self.loss_threshold = 3  # You can adjust this value as needed
    ...
    def retrain_model(self, asset, data):
        ...
        self.consecutive_losses[asset] = 0  # Reset the counter after retraining
    ...
    def get_state(self, history):
        ...
        rsi = self.calculate_rsi(history['close']).iloc[-1]  # Get the last RSI value
        macd = self.calculate_macd(history['close']).iloc[-1]  # Get the last MACD value
...
def send_discord_message(message_type, content, mention_content=""):
    time.sleep(4)  # 7-second cooldown
    try:
